...
apt update;
apt install gcc pkg-config sudo libssh-gcrypt-dev libgnutls28-dev  libglib2.0-dev libpcap-dev libgpgme-dev bison libksba-dev libsnmp-dev libgcrypt20-dev redis-server git python-pip cmake;
sleep 5;
...
sudo -u openvas cp redis-openvas.conf /etc/redis/;
sudo -u openvas chown redis:redis /etc/redis/redis-openvas.conf;
echo "db_address = /run/redis-openvas/redis.sock" > /etc/openvas/openvas.conf;
sudo -u openvas systemctl start redis-server@openvas.service;
